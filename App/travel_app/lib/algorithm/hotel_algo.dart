...
Future<void> main() async {
  final df = await DataFrame.fromCsv('App/travel_app/assets/database/hotels.csv' , verbose: true);
  
  String input_city= "Jaipur";
  int input_budget = 400;
  int input_days = 3; 
  List input_amenities = ['Spa', 'Pool', 'Restaurant' ,'Bar', 'Parking included'];
  double bugdetperday = input_budget/input_days;

  List shortlist = [];
  for(int i=0; i<df.length; i++){
     String st_price = df.colRecords('hotel_price',offset: i, limit: i+1)[0].toString();
     double db_price = double.parse(st_price);
    // ignore: unrelated_type_equality_checks
    if(df.colRecords('city',offset: i, limit: i+1)[0].toString() == input_city &&  db_price < bugdetperday){
      shortlist.add(i);
    }
  }
    List hotel_score = [];
    ...
    for(int i=0; i<shortlist.length;i++){
      ...
      String amn = df.colRecords('amenities',offset: i, limit: i+1).toString().replaceAll("[","");
        amn = amn.replaceAll("]","");
        amn = amn.replaceAll("'","");
    ...
    var map = new Map();
    var shortlisted_scores = <String,List>{'5_hotels':[], '4_hotels':[], '3_hotels':[], '2_hotels':[]};
   
    for(MapEntry hotel in shortlist2.entries){
      if(hotel.value==5)
        shortlisted_scores['5_hotels']!.add(hotel.key);
      if(hotel.value==4)
        shortlisted_scores['4_hotels']!.add(hotel.key);
      if(hotel.value==3)
        shortlisted_scores['3_hotels']!.add(hotel.key);
      if(hotel.value==2)
        shortlisted_scores['2_hotels']!.add(hotel.key);
    }
    List hotel_recc = [];
    ...
    for(int i in h5)
      hotels_5[i] = double.parse(df.colRecords('hotel_rating',offset: i, limit: i+1)[0].toString());
    ...
    for(int i in h4)
      hotels_4[i] = double.parse(df.colRecords('hotel_rating',offset: i, limit: i+1)[0].toString());
    ...
    for(int i in h3)
      hotels_3[i] = double.parse(df.colRecords('hotel_rating',offset: i, limit: i+1)[0].toString());
    ...
    for(int i in h2)
      hotels_2[i] = double.parse(df.colRecords('hotel_rating',offset: i, limit: i+1)[0].toString());
    ...
    for(var hotel_id in hotel_recc)
      return_id.add(int.parse(df.colRecords('hotel_id',offset: hotel_id, limit: hotel_id+1)[0].toString()));
